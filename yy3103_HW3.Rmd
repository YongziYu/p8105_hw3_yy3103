...
ggplot(two_panel, aes(x=data_value,y=year,color=response))+geom_point()+
  facet_grid(year ~.)
```
-In 2002,there are `r pull(location_2002,state)` that are observed at 7 or more locations. In 2010,there are `r pull(location_2010,state)` that are observed at 7 or more locations. 
